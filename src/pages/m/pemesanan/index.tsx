...
export default function LoginPage({ profile: initialProfile }: Props) {
  ...
  const onSubmitHandler = (inputs: MemberOrderInputTypes) => {
    mutation.mutate(inputs, {
      onSuccess(data: any) {
        ...
        formReset();
        router.push(
          `/m/pemesanan/berhasil?laundryQueue=${data?.order?.laundryQueue?.laundryQueueId}`
        );
      },
      onError(error: any) {
  ...
  return (
    <>
      ...
      <MemberPageHeader title="Pemesanan" />
            ...
            <Col xs={{ span: 12, order: 0 }}>
              <div className="pb-3">
                <Alert variant="warning">
                  Anda saat ini tidak dapat melakukan pemesanan. Mohon lengkapi
                  informasi pribadi Anda sebelum melanjutkan pemesanan.{' '}
                  <Link href="/m/profile" passHref legacyBehavior>
                    <a href="/m/profile" className="alert-link">
                      ...
                      {dataServiceQuery?.data?.map((srv) => {
                        return (
                          <label
                            ...
                            className="col-sm-6 col-md-6 col-lg-4 p-1 cursor-pointer"
                          >
                            <input
                              className="d-none"
                              type="checkbox"
                            ...
                            <div className="checkbox-service-card p-3 text-wrap h-100 border w-100 rounded shadow-none ">
                              ...
                              <p className="my-0 text-center text-wrap text-sm ">
                                {srv.name}
                              </p>
                            </div>
                          </label>
...
LoginPage.layout = WebMemberLayout;
