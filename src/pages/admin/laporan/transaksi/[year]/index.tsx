...
import TableReportTrxMonth from '@components/Tables/Report/TableReportTrxMonth';

interface Props extends IPageProps {}

export default function LaporanTransaksiYearPage({ userAuth }: Props) {
  const router = useRouter();
  const year = (router.query?.year as string) || null;
  const TITLE = `Laporan Transaksi Tahun ${year} | ${APP_NAME}`;
  ...
  return (
    <>
      ...
      <Container fluid className="p-0">
        <h1 className="h3 mb-3">Laporan Transaksi Tahun {year}</h1>
        <Row className="row">
          <Col xs={12}>
            {/* <TableReportTransaction typeQueryKey="transactions" /> */}
            {year ? <TableReportTrxMonth year={year} /> : null}
          </Col>
        </Row>
      </Container>
    </>
...
LaporanTransaksiYearPage.layout = AdminLayout;
